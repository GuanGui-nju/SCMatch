...
from models.catmatch.catmatch import CatMatch
...
config_file = "./config/catmatch.yaml"  # catmatch_bs, catmatch
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = CatMatch(_net_builder,
                     args.num_classes,
                     args.ema_m,
                     args.T,
                     args.p_cutoff,
                     args.ulb_loss_ratio,
                     args.contrast_loss_ratio,
                     args.num_train_iter,
                     args.min_clust_K,
                     args.max_clust_K,
                     args.method_rampup,
                     args.rampup_ratio,
                     args.clust_cutoff,
                     args.clust_cont_temp,
                     args.cluster_from_iter,
                     args.flag_use_std_cont,
                     not args.soft_label,
    ...
    ## set SGD and cosine lr on CatMatch 
    ...
    logger.info(f"  CatMatch   >> K:{args.min_clust_K}-{args.max_clust_K}. Rampup:{args.method_rampup} RampRatio:{args.rampup_ratio}")
    logger.info(f"             >> ClustFrom:{args.cluster_from_iter} ClustSimThr:{args.clust_cutoff}. Std-Contrast:{args.flag_use_std_cont}")
    logger.info(f"             >> Contrast-Temp:{args.clust_cont_temp} Contrast-LossW:{args.contrast_loss_ratio}")
    logger.info("-"*66)
