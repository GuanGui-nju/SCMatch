...
from .cat_utils import consistency_loss, Get_Scalar, ce_loss
from .cat_utils import AverageMeter, accuracy
from .cat_utils import contrast_loss_clust, contrast_loss_std
from .cat_utils import exp_rampup, linear_rampup, step_rampup
...
#         amp_cm = autocast if args.amp else contextlib.nullcontext
        ...
        for (x_lb, y_lb), (x_ulb_w, x_ulb_s, y_ulb) in zip(self.loader_dict['train_lb'], self.loader_dict['train_ulb']):
            ...
            with amp_cm():
                ...
                unsup_loss, mask, num_high, num_high_accurate, dist_ulb_all, dist_ulb_high = consistency_loss(logits_x_ulb_w, 
                    logits_x_ulb_s, y_ulb, 'ce', T, p_cutoff,
                    use_hard_labels=self.use_hard_label)

                # contrast_loss
                curr_K = "N/A"
                if self.it < self.cluster_from_iter:
                    if self.flag_std_cont:
                        ctr_loss = contrast_loss_std(num_ulb, self.clust_cont_temp, logits_x_ulb_w, 
                            logits_x_ulb_s, flag_z1_detach=True)
                    else: 
                        curr_K = self.min_k + np.floor( (self.max_k - self.min_k) * self.func_rampup(self.it) )
                        curr_K = step_rampup(int(curr_K))
                        ctr_loss = contrast_loss_clust(logits_x_ulb_w, logits_x_ulb_s, feats_ulb_w, 
                            curr_K, n_feats=ndim_feat, temperature=self.clust_cont_temp, 
                            clust_cutoff=self.clust_cutoff, device_id=self.device_id)
                else:
                    ctr_loss = torch.tensor(0.0)
...
#                 self.optimizer.step()
            ...
            if self.it % self.num_eval_iter == 0:
                ...
                # self.print_fn("-"*100)
                self.print_fn(" >> TRAIN  Iter:{} lossX:{:.3f} lossU:{:.3f} lossC:{:.3f} correctU:{:.2f}/{:.2f} Pseudo:{:.3f}/{:.3f} K:{} R:{:.3f} T:{:.3f}".format(
                    self.it, meter_loss_x.avg, meter_loss_u.avg, meter_loss_c.avg, 
                    tb_dict["train/pseudo_amount_acc"], tb_dict['train/pseudo_amount'], 
                    tb_dict["train/quantity"], tb_dict["train/quality"], curr_K, tb_dict['lr'], tb_dict["train/run_time"]))
