...
#     print(logits_softmax.shape, feats_ulb_w.shape, feats_ulb_w.dtype, n_clusts)
    ...
    # 2) 1. kmeans
    clf_kmean = Kmeans(n_clusts, n_feats, device_id)
#     print("="*20, feats_ulb_w.dtype, feats_ulb_w.shape)
#     print(feats_ulb_w)
    clf_kmean.cluster(feats_ulb_w)
    
    # 2) 2. get labels
    pseudolabels = []
    image_indexes = []
    for cluster, images in enumerate(clf_kmean.images_lists):
        image_indexes.extend(images)
        pseudolabels.extend([cluster] * len(images))
    new_pseudo = [v[0] for v in sorted(zip(pseudolabels, image_indexes), key=lambda v: v[1])]
    if n_clusts != len(clf_kmean.images_lists):
    ...
    del pseudolabels, image_indexes

    # 3) get distribution for each cluster
    clust_dist = []
    for each in clf_kmean.images_lists:
        tmp_dist_clust = logits_softmax[each]
        tmp_thred = mask_bool[each]
#         print("="*20, tmp_thred)
        if tmp_thred.sum() < 1:
            return torch.tensor(0.0)
        
        tmp_dist = tmp_dist_clust[tmp_thred]
        clust_dist.append(tmp_dist.mean(0))
    dists = torch.stack(clust_dist)
    del clust_dist

    # test
#     print(pseudo_onehot.shape, type(pseudo_onehot), pseudo_onehot.dtype, dists.shape, type(dists), dists.dtype)
...
#     print("!"*20, sim_probs.dtype, sim_probs.shape, sim_probs)
#     print("!"*20, sim_probs.shape, pseudo_onehot.shape, mask.shape)
    loss_c = - ( (torch.log(sim_probs + 1e-6) * pseudo_onehot)).sum(1)
    loss_c = loss_c * mask_bool.float()
#     print("!"*20, loss_c.shape, mask.shape)
    loss_c = loss_c.mean()
...
def contrast_loss_std(batch_size, temperature, z1, z2, flag_z1_detach=True):
    if flag_z1_detach:
        z1 = z1.detach()
    prob1 = torch.softmax(z1, dim=-1)
    prob2 = torch.softmax(z2, dim=-1)
